...
export const PointsTable: React.FC<{ league: League; group: Group }> = ({ league, group }) => {
    ...
    const pointsKey = league.id && group.id ? `${league.id}_${group.id}` : null;
    ...
    useEffect(() => {
      ...
      (async () => {
        ...
        if (res.payload) {
          dispatch(setPoints({ leagueId: league.id!, groupId: group.id!, points: res.payload.points }));
        }
      })();
    }, [league?.id, group?.id, dispatch]);
